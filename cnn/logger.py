...
class Logger():
    ...
    def save(self):
        fname = pjoin('../logs', self._model_name, self._model_name + '.log')
        with open(fname, 'wb') as f:
            f.write("model_name: {}\n".format(self._model_name).encode('charmap'))
            f.write("batch_size: {}\n".format(self._batch_size).encode('charmap'))
            f.write("num_epochs: {}\n".format(self._epochs).encode('charmap'))
            f.write("best_epoch: {}\n".format(self._best_epoch).encode('charmap'))

            f.write('train info: \n'.encode('charmap'))
            for info in self._train_log:
                f.write("{0:d} {1:d} {2:.4f} {3:.4f}\n".format(
                    info[0], info[1], info[2], info[3]).encode('charmap'))

            f.write('valid info: \n'.encode('charmap'))
            for info in self._valid_log:
                f.write("{0:d} {1:.4f} {2:.4f}\n".format(
                    info[0], info[1], info[2]).encode('charmap'))
