...
import argparse
from cnn.encoder import VSE, clip_grad_norm, FusionNet, FusionNet_scale
...
def main():
    ...
    parser.add_argument('--logger_name', default='/mnt/data4/fusion_scaled/log',
                         help='Path to save  Tensorboard log.')
    parser.add_argument('--save_path', default='/mnt/data4/fusion_scaled',
                         help='Path to save the model ')
    #default='/mnt/data4/fusion_onway2/savepoint.pth.tar'
    #/mnt/data4/fusion_onway/save/savepoint_5533_on_62588it.pth.tar
    parser.add_argument('--resume', #default='/mnt/data4/fusion_onway2/savepoint.pth.tar', type=str, metavar='PATH', #default='/mnt/data4/fusion_onway/save/savepoint_5533_on_62588it.pth.tar'
    ...
    parser.add_argument('--params_file', default="/mnt/data2/betty/webvision_train/results/resnet50/5000classes_onemonth/model_best.tar",
                         help='weights resnet path')
   ...
   # tensorboard --logdir='/mnt/data4/fusion_onway//log_2' --port=6006


    model = FusionNet_scale(opt=opt)
    ...
    val_loader = torch.utils.data.DataLoader(dataset=val_dataset, batch_size= opt.batch_size, pin_memory=True, shuffle=False)
...
# Train the Model
    for epoch in range(opt.num_epochs):
        ...
        ave_loss, ave_acc, ave_top5 = validate(opt, val_loader, model, criterion, epoch, logger, opt.val_step, opt.log_step) 
        print("comparing ave_acc", ave_top5, best_rsum )
...
def train(opt, train_loader, model, epoch,  optimizer, criterion, logger):
    ...
    for i, train_data in enumerate(train_loader):
        ...
        #print("we consider", inputs.shape, labels.shape)
        ...
        if model.module.Eiters % opt.log_step == 0:
            ...
            logging.info(
                ...
                'Loss {loss:.3f} \t'
                'Nb_img {nb_img}\t'
                .format(epoch, i, len(train_loader), batch_time=time.time()-end,
...
def validate(opt, valid_loader, model, criterion, epoch, logger, print_freq=1, save_freq=10000):
    print("Start validation..")
    # switch to evaluate mode
    model.eval()
    top1 = AverageMeter()
    top5 = AverageMeter()
    top1_comp = AverageMeter()
    top5_comp = AverageMeter()    
...
#for the confusion matrix
    ...
    params = torch.load(opt.params_file)
    fc_test = nn.Linear(2048, 5607)
    fc_test.weight.data = params['state_dict']['module.fc.weight'].clone()
    fc_test.bias.data = params['state_dict']['module.fc.bias'].clone()

    for idx, data in enumerate(valid_loader, 0):
        num_batch = idx + 1
        inputs, labels, img_id = data 
        #print("labels", labels, img_id)      
        ...
        resnet_feat=torch.empty(len(inputs),2048)
        for id, feat_concat in enumerate(inputs):
            #print("check", feat_concat[:2048].shape, feat_concat[2048:].shape)
            resnet_feat[id,:] = feat_concat[:2048]
        output_comp = fc_test(resnet_feat.cuda())

        loss = criterion(output, labels)

        value, preds = torch.max(output, 1)
        value_comp, preds_comp = torch.max(output_comp, 1)
        ...
        #for top5 acc
        acc1, acc5 = accuracy(output, labels, topk=(1, 5))
        acc1_comp, acc5_comp = accuracy(output_comp, labels, topk=(1, 5))
        ...
        correct = (preds == labels).sum()
        correct_comp = (preds_comp == labels).sum()
        top1.update(acc1[0], inputs.size(0))
        top5.update(acc5[0], inputs.size(0))
        top1_comp.update(acc1_comp[0], inputs.size(0))
        top5_comp.update(acc5_comp[0], inputs.size(0))
        top_5_classes = top_5_classes.t()
        correct5 =  top_5_classes.eq(labels.view(1, -1).expand_as(top_5_classes))
        #print('details', top_5_classes , 'gtruth', labels )
        #print('correct whole', preds, preds_comp, labels, img_id)
        ...
        if idx % print_freq == (print_freq - 1):
            print( 'New@1 ({top1.avg:.3f})\t' 
                'Old@1  ({top1_comp.avg:.3f})\t' 
                 'New@5  ({top5.avg:.3f})\t' 
                 'Old@5  ({top5_comp.avg:.3f})\t' .format(top1_val=top1.val/float(labels.size(0)),
                  top5_val=correct_5.item()/float(labels.size(0)),
                  top1=top1, top5=top5, top1_comp=top1_comp, top5_comp=top5_comp  ))
    ...
    #plt.savefig('/mnt/data2/betty/webvision_train/results/resnet50/confusion_matrix_cls_names.png')
        ...
        if idx % save_freq == (save_freq - 1):
            np.savetxt(os.path.join(opt.save_path, "all_pred.csv"), all_pred_m, delimiter=",")
            print('all_pred_m:', all_pred_m.shape )

            print('[valid {0:d}] ' 'loss: {1:.4f}\t' 'acc: {2:.4f}\t'  'Time {batch_time:.3f} \t'
    ...
    print(cm.shape)
    np.savetxt(os.path.join(opt.save_path, "confusion_matrix_training.csv"), cm, delimiter=",")
