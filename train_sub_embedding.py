import pickle
import os
os.environ["CUDA_VISIBLE_DEVICES"] =  "3" #we give this number as the device id we use
...
def main():
    ...
    parser.add_argument('--log_step', default=25, type=int,
                         help='Number of steps to print and record the log.')
    parser.add_argument('--val_step', default=500, type=int,
                         help='Number of steps to run validation.')
    parser.add_argument('--logger_name', default='/mnt/data4/embedding_training_result_sub',
    ...
    parser.add_argument('--save_fq', help='saving backup fq' , default='500', type=int )
    ...
    train_dataset_google = EmbeddedFolder( root_json=pjoin(opt.root_json, "google"), root_img=pjoin(opt.root_img, "google_solo"), w2v_model=w2v_model, transform=train_transform, dict_=query_synset_dic, part=True)
    ...
    train_dataset_flickr = EmbeddedFolder(root_json=pjoin(opt.root_json, "flickr"), root_img=pjoin(opt.root_img, "flickr_solo"), w2v_model=w2v_model, transform=train_transform, dict_=query_synset_dic, part=True)
    ...
    #val_dataset= PrecompImg(root_img="/mnt/data3/web_feature_training/")
    ...
    #print(model.img_enc, "\n and txt enc", model.txt_enc)
        #prev_gpus = opt.gpus
    # gpus = [int(i) for i in opt.gpus.strip('[]').split(',')]
    # #print(gpus)
...
# Train the Model
    for epoch in range(opt.num_epochs):
        ...
        # try:
        #     save_encoding(model, val_loader, opt)
        # except:
        #     print("save skipped")
        rsum=best_rsum
...
def train(opt, train_loader, model, epoch, val_loader):
    ...
    for i, train_data in enumerate(train_loader):
        ...
        #check GRU training
        #print("GRU learning", model.txt_enc.rnn.state_dict())
        ...
        #print("writer is", writer, loss_value.data.item(), (epoch*len(train_loader)+i))
